...
var catalog: Array = []
var settings := {"sensitivity": 1.0, "volume": 0.65, "shake": 0.55, "zoom": 24.0, "fullscreen": false}
var _modal_kind := ""
...
func _build_hud() -> void:
	...
	_reload_bar.hide()
	var hints := _label("WASD 移动    右键 瞄准    R 换弹    4—8 投掷    TAB 背包    B 商店    E 交互    ESC 暂停",11,Color(0.93,0.91,0.86,0.8))
	hints.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	hints.add_theme_color_override("font_shadow_color",Color.BLACK)
	hints.add_theme_constant_override("shadow_outline_size",0)
	_anchor(hints,0.5,1,0.5,1,-470,-35,470,-10)
	hud.add_child(hints)
...
func _open_settings() -> void:
	var panel := _open_panel("设置与操作","以鼠标朝向瞄准。站定、短点射与利用掩体，是取得优势的关键。","settings",900,578)
	_at(_label("操作",17,PAPER),panel,32,131,340,29)
	var controls := ["W A S D       移动","鼠标左 / 右键   射击 / 稳定瞄准","R                     更换弹匣","1—3 / 4—8      武器 / 投掷物","TAB / B          物品栏 / 商店","E                     安放 / 拆除炸弹","F / G               拾取 / 丢弃武器","滚轮                 调整镜头范围","ESC                 暂停 / 关闭面板"]
	for i in controls.size():
		_at(_label(controls[i],13,MUTED),panel,34,173+i*34,397,29)
	_at(_label("偏好",17,PAPER),panel,476,131,370,29)
	_setting_slider(panel,"音量","volume",0,1,0.05,476,176)
	_setting_slider(panel,"镜头范围","zoom",16,34,1,476,247)
	_setting_slider(panel,"射击震动","shake",0,1,0.05,476,318)
	var fullscreen := CheckButton.new()
	fullscreen.text = "全屏显示"
	fullscreen.button_pressed = bool(settings.get("fullscreen",false))
	fullscreen.toggled.connect(func(value: bool): settings.fullscreen = value; settings_changed.emit(settings.duplicate()))
	_at(fullscreen,panel,476,397,343,44)
	var done := _button("完成",_close_panel_by_button,true)
	_at(done,panel,476,489,343,47)

func _setting_slider(parent: Control, title: String, key: String, min_value: float, max_value: float, step: float, x: float, y: float) -> void:
	_at(_label(title,13,MUTED),parent,x,y,260,26)
	var value_label := _label(_setting_value_text(key,float(settings[key])),12,PAPER,true)
	value_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_at(value_label,parent,x+265,y,77,26)
	...
	slider.value_changed.connect(func(value: float): settings[key] = value; value_label.text = _setting_value_text(key,value); settings_changed.emit(settings.duplicate()))
	_at(slider,parent,x,y+29,342,20)

func _setting_value_text(key: String, value: float) -> String:
	return "%d m" % roundi(value) if key == "zoom" else "%d%%" % roundi(value*100)
